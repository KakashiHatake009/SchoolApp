  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      <header className="flex items-center justify-between px-6 py-3 border-b border-gray-100">
        ...
        <span className="text-base text-gray-500">{t('teacher.pageTitle')}</span>
        <LanguageSwitcher />
      </header>
...
export default function TeacherManagePage() {
  ...
  const labelTitle = (tag: string, name: string, v: boolean | null | undefined) => {
    const state = v === true ? t('legend.present') : v === false ? t('legend.absent') : t('legend.noResponse')
  ...
  if (step === 'code') {
    const handleEnter = async () => {
      if (!code.trim()) {
        setCodeError(t('teacher.codeRequired'))
        return
      }
      setCodeLoading(true)
      setCodeError('')
      try {
        ...
        setCodeError(err instanceof Error ? err.message : t('teacher.codeInvalid'))
      } finally {
    ...
    return (
      <PageShell schoolName={schoolName} eventName={eventName} schoolLogo={schoolLogo}>
        ...
        <div className="flex flex-col items-center justify-center mt-8">
          ...
          <input
            type="text"
            placeholder={t('teacher.codePlaceholder')}
            value={code}
  ...
  if (step === 'slots') {
    ...
    const teacherData = teacherSession?.teacher
    const t1Name = [teacherData?.salutation, teacherData?.firstName, teacherData?.surname].filter(Boolean).join(' ')
    const t2Name = [teacherData?.salutation2, teacherData?.firstName2, teacherData?.surname2].filter(Boolean).join(' ')
    ...
    const slotBtn = (slot: AppointmentSlot) => {
      ...
      const btnTitle = isBooked
        ...
        : hasSecondTeacher
        ? myPresence === false
          ? t('teacher.clickToRelease')
          : t('teacher.clickToBlock')
        : slot.status === 'disabled'
        ? t('teacher.clickToActivate')
        : t('teacher.clickToBlock')
      ...
      <PageShell schoolName={schoolName} eventName={eventName} schoolLogo={schoolLogo}>
        {myName && (
          <p className="text-sm text-gray-500 mb-4">
            {t('teacher.loggedInAs')} <span className="font-semibold text-gray-700">{myName}</span>
            {hasSecondTeacher && <span className="ml-2 text-gray-400">{t('teacher.ofTeachers', { tag: myTag })}</span>}
            {teacher?.klasse && <span className="ml-2 text-gray-400">{t('teacher.classLabel', { class: teacher.klasse })}</span>}
            {teacher?.roomNo && <span className="ml-2 text-gray-400">{t('teacher.roomLabel', { room: teacher.roomNo })}</span>}
          </p>
        )}

        <h2 className="font-bold text-gray-800 mb-1">{t('teacher.editSlots')}</h2>
        <p className="text-sm text-gray-500 mb-5">
          {t('teacher.editSlotsDesc')}
        </p>
          ...
          <div className="flex-1 w-full rounded overflow-hidden">
            ...
            >
              ...
              {weekDates.every((date) => !(slotsByDate[date]?.length)) && (
                <div className="col-span-full py-6 text-center text-sm text-gray-400">{t('teacher.noSlots')}</div>
              )}
            </div>
          </div>
            ...
            {hasSecondTeacher ? (
              <>
                <div>
                  ...
                  <div className="space-y-1 text-gray-700">
                    <div className="leading-snug">
                      ...
                      {myIndex === 1 && <span className="text-[#1565c0] ml-1">{t('legend.you')}</span>}
                      <br /><span className="text-gray-500">{t1Name}</span>
                    </div>
                    <div className="leading-snug">
                      <span className="font-medium">T2</span>
                      {myIndex === 2 && <span className="text-[#1565c0] ml-1">{t('legend.you')}</span>}
                ...
                <div className="border-t border-gray-100 pt-2">
                  <p className="font-medium text-gray-500 mb-1">{t('legend.yourAvailability')}</p>
                  <div className="space-y-1 text-gray-700">
                    <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-[#90caf9] shrink-0" />{t('legend.available')}</span>
                    <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-gray-700 shrink-0" />{t('legend.blocked')}</span>
                    <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-[#f5a623] shrink-0" />{t('legend.booked')}</span>
                  </div>
                </div>

                <div className="border-t border-gray-100 pt-2">
                  <p className="font-medium text-gray-500 mb-1">{t('legend.teacherStatus')}</p>
                  <div className="space-y-1 text-gray-700">
                   <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-[#16A34A] shrink-0" />{t('legend.present')}</span>
                    ...
                    <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-[#9CA3AF] shrink-0" />{t('legend.noResponse')}</span>
                  </div>
                </div>
              </>
            ) : (
