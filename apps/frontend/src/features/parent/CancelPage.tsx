...
export default function CancelPage() {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      setCancelError(err instanceof Error ? err.message : t('cancel.cancelError'))
    } finally {
      setCancelling(false)
    }
  }
  ...
  const handleReschedule = async () => {
    ...
    try {
      ...
      setRescheduleError(err instanceof Error ? err.message : t('cancel.rescheduleError'))
    } finally {
  ...
  const activeTeacher = teachers.find((tc) => tc.id === activeTeacherId)
        ...
        <div className="flex-1 w-full rounded overflow-hidden">
          ...
          >
                ...
                {(slotsByDate[date] ?? []).map((slot) => {
                  ...
                  return (
                    <div key={slot.id} className="py-1.5 px-1 flex flex-col items-center gap-0.5">
                      ...
                      {isCurrentBooking && (
                        <span className="text-[8px] text-green-600 font-semibold leading-none">{t('cancel.yourSlot')}</span>
                      )}
                      {cancelHasT2 && (
                        <div className="flex gap-1">
                          <span
                            className={`text-[8px] font-semibold leading-none ${labelColor(slot.teacher1Present)}`}
                            title={`T1 – ${cancelT1Name}: ${slot.teacher1Present === true ? t('legend.present') : t('legend.absent')}`}
                          >T1</span>
                          <span
                            className={`text-[8px] font-semibold leading-none ${labelColor(slot.teacher2Present)}`}
                            title={`T2 – ${cancelT2Name}: ${slot.teacher2Present === true ? t('legend.present') : t('legend.absent')}`}
                          >T2</span>
                        </div>
                      )}
                    </div>
            ...
            {weekDates.every((date) => !(slotsByDate[date]?.length)) && (
              <div className="col-span-full py-6 text-center text-sm text-gray-400">{t('cancel.noSlots')}</div>
            )}
          </div>
        </div>
        ...
        <div className="hidden sm:block w-48 shrink-0 text-xs border border-gray-200 rounded-lg p-3 space-y-3">
          ...
          <div>
            ...
            <div className="space-y-1 text-gray-700">
              <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-green-500 shrink-0" />{t('cancel.yourSlot')}</span>
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      <header className="flex items-center justify-between px-6 py-3 border-b border-gray-100">
        ...
        <span className="text-base text-gray-500">{t('cancel.title')}</span>
        <LanguageSwitcher />
      </header>

      <main className={`flex-1 px-6 py-10 w-full mx-auto ${mode === 'reschedule' ? 'max-w-4xl' : 'max-w-xl'}`}>
        {isLoading && <p className="text-sm text-gray-400 text-center">{t('cancel.loadingDetails')}</p>}

        {isError && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
            <p className="text-red-600 font-medium">{t('cancel.notFound')}</p>
            <p className="text-sm text-gray-500 mt-2">{t('cancel.notFoundDesc')}</p>
          </div>
        )}
        ...
        {cancelled && (
          <div className="bg-[#dde8ee] rounded-lg p-6 text-center">
            <p className="font-bold text-gray-800 text-base mb-2">{t('cancel.cancelled')}</p>
            <p className="text-sm text-gray-600">{t('cancel.cancelledDesc')}</p>
          </div>
        )}
        ...
        {rescheduled && (
          <div className="bg-[#dde8ee] rounded-lg p-6 text-center">
            <p className="font-bold text-gray-800 text-base mb-2">{t('cancel.rescheduled')}</p>
            <p className="text-sm text-gray-600">
              {t('cancel.rescheduledDesc', { date: selectedSlot?.date, time: selectedSlot?.time })}
            </p>
          </div>
        )}
        ...
        {booking && !cancelled && !rescheduled && (
          <>
            ...
            <div className="bg-[#dde8ee] rounded-lg p-6 mb-6">
              <p className="font-bold text-gray-800 mb-4">{t('cancel.yourDetails')}</p>
              <div className="space-y-1 text-sm text-gray-700">
                <p><span className="text-gray-500">{t('cancel.event')}:</span> {booking.event.name}</p>
                {booking.slot && (
                  <>
                    <p><span className="text-gray-500">{t('cancel.teacher')}:</span> {booking.slot.teacher.salutation} {booking.slot.teacher.surname}</p>
                    {booking.slot.teacher.roomNo && (
                      <p><span className="text-gray-500">{t('cancel.room')}:</span> {booking.slot.teacher.roomNo}</p>
                    )}
                    <p><span className="text-gray-500">{t('cancel.date')}:</span> {booking.slot.date}</p>
                    <p><span className="text-gray-500">{t('cancel.time')}:</span> {booking.slot.time}</p>
                  </>
                )}
                <p><span className="text-gray-500">{t('cancel.name')}:</span> {booking.parentFirstName} {booking.parentSurname}</p>
              </div>
            </div>

            {booking.status === 'CANCELLED' ? (
              <div className="bg-gray-100 rounded-lg p-6 text-center">
                <p className="text-gray-600 font-medium">{t('cancel.alreadyCancelled')}</p>
              </div>
            ) : mode === 'view' ? (
              <>
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                  ...
                  >
                    {cancelling ? '…' : t('cancel.cancelBooking')}
                  </button>
                </div>
              </>
            ) : (
              ...
              (() => {
                ...
                const AbsentWarning = () => hasAbsentWarning ? (
                  <div
                    className="mt-3 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded px-4 py-2"
                    dangerouslySetInnerHTML={{ __html: t('absent.warning', { name: absentName }) }}
                  />
                ) : null
                return <>
                ...
                <div className="hidden sm:flex gap-8">
                  ...
                  <div className="w-44 flex-shrink-0">
                    <p className="text-sm text-gray-600 mb-3">{t('cancel.selectTeacher')}</p>
                    <div className="space-y-1">
                      {teachers.map((tc) => {
                        const isCurrentTeacher = tc.id === booking?.slot?.teacher?.id
                        return (
                          <button
                            key={tc.id}
                            onClick={() => handleTeacherSelect(tc.id)}
                            className={`w-full text-left text-sm px-2 py-1.5 rounded cursor-pointer transition-colors ${
                              activeTeacherId === tc.id
                          ...
                          >
                            ...
                            {isCurrentTeacher && (
                              <span className="text-[10px] text-green-600 font-semibold leading-none">{t('cancel.yourSlot')}</span>
                            )}
                          </button>
                        )
                      })}
                ...
                <div className="sm:hidden">
                  <p className="text-sm text-gray-600 mb-2">{t('cancel.selectTeacher')}</p>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {teachers.map((tc) => (
                      ...
                      >
                        ...
                        {tc.id === booking?.slot?.teacher?.id && (
                          <span className="block text-[10px] text-green-600 font-semibold leading-none mt-0.5">{t('cancel.yourSlot')}</span>
                        )}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex justify-between mt-6 gap-3">
                  ...
                  >
                    {rescheduling ? '…' : t('cancel.confirmNew')}
                  </button>
                </div>
              </>
              })()
