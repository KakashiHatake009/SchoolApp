  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      <header className="flex items-center justify-between px-6 py-3 border-b border-gray-100">
        ...
        <span className="text-base text-gray-500">{navTitle ?? t('booking.title')}</span>
        <LanguageSwitcher />
      </header>
...
function NavButtons({
  ...
  return (
    <div className={`flex mt-8 gap-4 ${onBack ? 'justify-between' : 'justify-end'}`}>
      {onBack && (
        <button onClick={onBack} className={btnCls}>{t('common.back')}</button>
      )}
      <button onClick={onNext} disabled={nextDisabled || nextLoading} className={btnCls}>
        {nextLoading ? '…' : resolvedNextLabel}
      </button>
...
export default function BookingPage() {
  ...
  if (!event.bookingActive || event.status !== 'published') {
    return (
      <PageShell navTitle={t('booking.title')} schoolName={schoolName} eventName={eventName} schoolLogo={schoolLogo}>
        <div className="flex justify-center mt-8">
          <div className="bg-[#dde8ee] rounded-lg px-10 py-10 text-center w-full max-w-md">
            <p className="font-bold text-gray-800 mb-3 text-base">{t('booking.notStarted')}</p>
            <p className="text-sm text-gray-600 leading-relaxed">
              {t('booking.notStartedDesc')}
            </p>
          </div>
        </div>
      </PageShell>
  ...
  if (step === 'email') {
    const handleSend = async () => {
      ...
      try {
        ...
        setOtpError(err instanceof Error ? err.message : t('booking.sendError'))
      } finally {
        setLoading(false)
      }
    }
  ...
  if (step === 'verify') {
    const handleVerify = async () => {
      ...
      try {
        ...
        setOtpError(err instanceof Error ? err.message : t('booking.invalidCode'))
      } finally {
        setLoading(false)
      }
    }
    ...
    const handleResend = async () => {
      ...
      try {
        ...
        setOtpError(t('booking.sendError'))
      } finally {
        setLoading(false)
      }
    }
    ...
    return (
      <PageShell navTitle={t('booking.title')} schoolName={schoolName} eventName={eventName} schoolLogo={schoolLogo}>
        <div className="flex justify-center mt-4">
          <div className="bg-[#dde8ee] rounded-lg px-10 py-8 text-center w-full max-w-md">
            <p className="font-bold text-gray-800 mb-2 text-base">{t('booking.enterCode')}</p>
            <p className="text-sm text-gray-600 mb-5">
              {t('booking.codeSentTo')} <strong>{email}</strong> {t('booking.codeSentSuffix')}
            </p>
            <input
              type="text"
  ...
  if (step === 'teachers') {
    const filtered = teachers.filter((tc) =>
      !teacherSearch ||
      `${tc.surname} ${tc.firstName} ${tc.surname2 ?? ''} ${tc.firstName2 ?? ''}`.toLowerCase().includes(teacherSearch.toLowerCase()),
    )
    return (
      <PageShell navTitle={t('booking.title')} schoolName={schoolName} eventName={eventName} description={description} schoolLogo={schoolLogo}>
        <div className="flex items-center gap-3 mb-4">
          <span className="font-medium text-gray-700 text-sm whitespace-nowrap">{t('booking.searchTeacher')}</span>
          <input
            value={teacherSearch}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#dde8ee] text-gray-700">
                <th className="px-4 py-3 text-left font-medium">{t('booking.teacherCol')} <span className="text-gray-400">↓</span></th>
                <th className="px-4 py-3 text-left font-medium">{t('booking.educatorCol')} <span className="text-gray-400">↓</span></th>
                <th className="px-4 py-3 text-left font-medium">{t('booking.roomCol')}</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 && (
              ...
              {filtered.map((tc, i) => (
                ...
                >
                  <td className="px-4 py-3 text-gray-800">
                    {tc.surname} {tc.firstName.charAt(0)}.
                  </td>
                  <td className="px-4 py-3 text-gray-600">
                    {tc.firstName2 && tc.surname2
                      ? `${tc.surname2} ${tc.firstName2.charAt(0)}.`
                      : '—'}
                  </td>
                  <td className="px-4 py-3 text-gray-600">{tc.roomNo}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  if (step === 'slots' || step === 'reschedule') {
    ...
    const DAY_SHORT = [t('days.mo'), t('days.tu'), t('days.mi'), t('days.th'), t('days.fr'), t('days.sa'), t('days.su')]
      ...
      <>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start">
          ...
          <div className="flex-1 w-full rounded overflow-hidden">
            ...
            >
                    ...
                    return (
                      <div key={slot.id} className="py-1.5 px-1 flex flex-col items-center gap-0.5">
                        <button
                          ...
                          title={isCurrentBooking ? t('slots.yourSlot') : isBooked ? t('slots.alreadyBooked') : isDisabled ? t('slots.notAvailable') : t('slots.selectSlot')}
                        ...
                        {isCurrentBooking && (
                          <span className="text-[8px] text-green-600 font-semibold leading-none">{t('legend.yourSlot')}</span>
                        )}
                        {hasT2 && (
                          <div className="flex gap-1">
                            <span
                              className={`text-[8px] font-semibold leading-none ${labelColor(slot.teacher1Present)}`}
                              title={`T1 – ${t1Name}: ${slot.teacher1Present === true ? t('legend.present') : t('legend.absent')}`}
                            >T1</span>
                            <span
                              className={`text-[8px] font-semibold leading-none ${labelColor(slot.teacher2Present)}`}
                              title={`T2 – ${t2Name}: ${slot.teacher2Present === true ? t('legend.present') : t('legend.absent')}`}
                            >T2</span>
                          </div>
                        )}
                      </div>
              ...
              {weekDates.every((date) => !(slotsByDate[date]?.length)) && (
                <div className="col-span-full py-6 text-center text-sm text-gray-400">{t('booking.noSlots')}</div>
              )}
            </div>
          </div>
          ...
          <div className="hidden sm:block w-48 shrink-0 text-xs border border-gray-200 rounded-lg p-3 space-y-3">
            <p className="text-xs font-semibold text-gray-700 uppercase tracking-wide">{t('legend.title')}</p>

            <div>
              <p className="font-medium text-gray-500 mb-1">{t('legend.availability')}</p>
              <div className="space-y-1 text-gray-700">
                {isReschedule && bookedSlot && (
                  <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-green-500 shrink-0" />{t('legend.yourSlot')}</span>
                )}
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-[#90caf9] shrink-0" />{t('legend.available')}</span>
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-[#f5a623] shrink-0" />{t('legend.booked')}</span>
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-gray-300 shrink-0" />{t('legend.blocked')}</span>
              </div>
            </div>
            ...
            {hasT2 && (
              <>
                <div className="border-t border-gray-100 pt-2">
                  <p className="font-medium text-gray-500 mb-1">{t('legend.teachers')}</p>
                  <div className="space-y-1 text-gray-700">
                ...
                <div className="border-t border-gray-100 pt-2">
                  <p className="font-medium text-gray-500 mb-1">{t('legend.attendance')}</p>
                  <div className="space-y-1 text-gray-700">
                    <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-[#16A34A] shrink-0" />{t('legend.present')}</span>
                    <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-[#EF4444] shrink-0" />{t('legend.absent')}</span>
                  </div>
                </div>
              </>
            )}
    ...
    const AbsentWarning = () => hasAbsentTeacherWarning ? (
      <div className="mt-3 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded px-4 py-2">
        <span dangerouslySetInnerHTML={{ __html: t('absent.warning', { name: absentName }) }} />
      </div>
    ) : null

    if (isReschedule) {
      return (
        <PageShell navTitle={t('booking.changeDate')} schoolName={schoolName} eventName={eventName} description={description} schoolLogo={schoolLogo}>
          <h2 className="font-bold text-gray-800 mb-4">{t('booking.newSlotTitle')}</h2>
          {otpError && <p className="text-sm text-red-500 mb-3">{otpError}</p>}
          <div className="hidden sm:flex gap-8">
            <div className="w-44 flex-shrink-0">
              <p className="text-sm text-gray-600 mb-4">{t('booking.selectNewSlot')}</p>
          ...
          <NavButtons
            ...
            nextLabel={t('booking.confirmSlot')}
    ...
    return (
      <PageShell navTitle={t('booking.title')} schoolName={schoolName} eventName={eventName} description={description} schoolLogo={schoolLogo}>
        <div className="hidden sm:flex gap-8">
          <div className="w-44 flex-shrink-0">
            <p className="text-sm text-gray-600 mb-4">{t('booking.selectSlot')}</p>
            <div className="space-y-1">
              {teachers.map((tc) => (
                <button
                  key={tc.id}
                  onClick={() => { setSelectedTeacher(tc); setSelectedSlot(null) }}
                ...
                >
                  <span className="block leading-tight">{tc.salutation} {tc.firstName} {tc.surname}</span>
                  {tc.firstName2 && tc.surname2 && (
                    <span className="block leading-tight text-xs opacity-75">{tc.salutation2} {tc.firstName2} {tc.surname2}</span>
                  )}
                </button>
              ))}
            </div>
        ...
        <div className="sm:hidden">
          <h2 className="font-bold text-gray-800 text-lg mb-4">{t('booking.selectSlot')}</h2>
          {CalendarGrid()}
          <AbsentWarning />
        </div>
  ...
  if (step === 'form') {
    ...
    const SalutationSelect = ({ value, onChange }: { value: string; onChange: (v: string) => void }) => (
      <select value={value} onChange={(e) => onChange(e.target.value)} className={selectCls}>
        <option value="">{t('booking.salutation')}</option>
        <option value="Hr.">{t('booking.mr')}</option>
        <option value="Fr.">{t('booking.mrs')}</option>
      </select>
    )

    return (
      <PageShell navTitle={t('booking.title')} schoolName={schoolName} eventName={eventName} description={description} schoolLogo={schoolLogo}>
        <h2 className="font-bold text-gray-800 mb-5">{t('booking.yourInfo')}</h2>

        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <SalutationSelect value={form.salutation} onChange={(v) => setForm((f) => ({ ...f, salutation: v }))} />
            <input placeholder={t('booking.firstName')} value={form.firstName} onChange={(e) => setForm((f) => ({ ...f, firstName: e.target.value }))} className={inputCls} />
            <input placeholder={t('booking.lastName')} value={form.surname} onChange={(e) => setForm((f) => ({ ...f, surname: e.target.value }))} className={inputCls} />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="hidden sm:block" />
            <input type="email" value={email} readOnly className={inputCls + ' bg-gray-50 cursor-default'} />
            <input placeholder={t('common.phone')} value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))} className={inputCls} />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="hidden sm:block" />
            <input placeholder={t('booking.childName')} value={form.childName} onChange={(e) => setForm((f) => ({ ...f, childName: e.target.value }))} className={inputCls} />
            <input placeholder={t('booking.class')} value={form.childClass} onChange={(e) => setForm((f) => ({ ...f, childClass: e.target.value }))} className={inputCls} />
          </div>
          ...
          {form.hasSecondPerson && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <input placeholder={t('booking.secondFirstName')} value={form.secondFirstName} onChange={(e) => setForm((f) => ({ ...f, secondFirstName: e.target.value }))} className={inputCls} />
              <input placeholder={t('booking.secondLastName')} value={form.secondSurname} onChange={(e) => setForm((f) => ({ ...f, secondSurname: e.target.value }))} className={inputCls} />
            </div>
          )}
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      <header className="flex items-center justify-between px-6 py-3 border-b border-gray-100">
        <img src="/logo.png" alt="WahlWeise" className="h-8 w-auto object-contain shrink-0" />
        <span className="text-base text-gray-500">{t('booking.bookingDetails')}</span>
        <LanguageSwitcher />
      </header>

      <main className="flex-1 px-4 sm:px-6 py-4 sm:py-6 w-full max-w-5xl mx-auto">
        <div className="bg-[#1565c0] text-white text-center font-bold py-4 px-6 rounded mb-8">
          {t('booking.confirmed', { eventName, schoolName })}
        </div>

        <div className="max-w-xl mx-auto space-y-4">
          <div className="bg-[#dde8ee] rounded-lg p-6">
            <p className="font-bold text-gray-800 mb-3">{t('booking.yourBooking')}</p>
            <p className="text-sm text-gray-700">
              {selectedTeacher?.salutation} {selectedTeacher?.surname}
            </p>
            {selectedTeacher?.roomNo && (
              <p className="text-sm text-gray-700">{t('cancel.room')}: {selectedTeacher.roomNo}</p>
            )}
            <p className="text-sm text-gray-700">{t('cancel.date')}: {selectedSlot?.date}</p>
            <p className="text-sm text-gray-700">{t('cancel.time')}: {selectedSlot?.time}</p>
          </div>

          <div className="bg-[#dde8ee] rounded-lg p-6">
            <p className="font-bold text-[#1565c0] mb-3">{t('booking.important')}</p>
            <p className="text-sm text-gray-700 mb-3">
              {t('booking.beOnTime')}
            </p>
            {cancelUrl && (
              <p className="text-sm text-gray-700">
                {t('booking.cancelOrChangeLink')}{' '}
