    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">We'll send you a reset code</p>
        </div>

        {sent ? (
          <div className="bg-white rounded-2xl border border-[#1565c0]/20 shadow-sm p-6 space-y-4 text-center">
            <p className="text-sm text-gray-700">
              If an account with that email exists, a reset code has been sent.
            </p>
            <Link
              to={`/reset-password?email=${encodeURIComponent(email)}`}
              className="inline-block text-sm text-[#1565c0] hover:underline"
            >
              Enter reset code
            </Link>
          </div>
